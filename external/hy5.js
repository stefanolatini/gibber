...
HY5.hydraClass = class HY5_HYDRA{
	constructor(hydra = 'hydra', synth = ''){
		if(window[hydra] == undefined){
			// In ambienti tipo Gibber, window.Hydra è una factory che restituisce
			// hydra.synth (non il renderer) e riusa sempre la stessa istanza.
			// Per istanze secondarie, usa HydraClass (la classe vera) se disponibile.
			if(window.HydraClass !== undefined && hydra !== 'hydra'){
				const _c = document.createElement('canvas')
				_c.width = window.innerWidth
				_c.height = window.innerHeight
				_c.style.cssText = 'position:absolute;top:0;left:0'
				document.body.appendChild(_c)
				window[hydra] = new window.HydraClass({canvas:_c, global:false, detectAudio:false})
			} else {
